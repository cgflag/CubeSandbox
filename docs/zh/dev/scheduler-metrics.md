...
3. 可行节点按 Profile 权重打分后，绑定最高分节点（确定性 argmax，同分按节点 ID 稳定打破）。生产 CubeMaster 只有在配置了 `scheduler.score` 并产生分数之后，才会按 `scheduler.priority_select_num` / `scheduler.least_select_name` 做截断与最终选择。当打分已启用且 `priority_select_num` 为 `1` 时，截断集合只有一个节点，生产选择与本 simulator 的 argmax 一致（同分打破规则可能不同）。当前发行配置启用了 filter，但没有 `score:` 块，因此生产会从过滤后的节点列表中选择，而不会走该 argmax 步骤。
...
生产 CubeMaster 只有在配置了 `scheduler.score` 并产生分数之后，才会按
...
因此生产会从过滤后的节点列表中选择，而不会走该 argmax 步骤。在本 simulator 的绑定
规则下，全量排序后再截断不会改变最终绑定节点，所以报告只暴露
